...
static int parse_arg(char *arg, command_template *command, bool is_destination) {
    int number_arg; /* stores number argument (if present) */
    int _register = EMPTY_FIELD;  /* stores the register number in command */
    int addressing_type = -1; /* addressing type found*/
